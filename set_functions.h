 ...
 * Author: vespine
 *
 * Created on April 28, 2018, 10:34 PM
 */
...
char display_array[NUM_ROWS][NUM_TLCS * 16];// 8 * 192 (64r-64g-64b)this is where the LED data is actually stored. Channel value is a float between 0 (0%) to 1.0f (100%).
...
void set(unsigned char row, unsigned char channel, int value) //very simple! set a single value to a specified spot on the matrix. 
{
...
static void set_rowRGB(unsigned char row, int R, int G, int B)
{
...
static void set_allRGB(int R, int G, int B)
{
...
static void set_xr(char x, char y, char z, int R, int G, int B)
{
